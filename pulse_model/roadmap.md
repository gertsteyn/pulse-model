...
The final frontier posture is tracked in [Frontier Strategy](./frontier_strategy.md). H6P1 closed the current internal source-response attempt as a diagnostic tool, so future novelty work should not be opened unless a genuinely new external principle supplies a conserved, coefficient-fixed, baseline-distinct response law.
...
| 7 | H6: classical spacetime from decohered pulse histories | Accepted with limits for reduced branch-decoherence bookkeeping and model comparison; H6S1 adds an executable weak-field quantum source-response diagnostic tool; H6S2 adds an ensemble-decomposition-invariance admissibility filter with a clean no-go subtheorem; H6S3 adds the no-free-branch-variance theorem and route ledger; H6S4-C adds a finite stochastic pulse-potential diagnostic with final verdict diagnostic tool; H6S4-Q adds a finite quantum mediator conditional derivation; H6P1 adds a finite gauge-invariant pulse-closure response diagnostic with final verdict diagnostic tool; full classical-spacetime emergence and a real source-response law remain blocked | `appendix/h6/decohered_pulse_histories_classical_spacetime.md`, `appendix/h6/quantum_source_response_discriminator.md`, `appendix/h6/causal_pulse_response_kernel.md`, `appendix/h6/no_free_branch_variance.md`, `appendix/h6/minimal_stochastic_pulse_geometry.md`, `appendix/h6/minimal_quantum_pulse_geometry.md`, `appendix/h6/pulse_closure_response_principle.md`, `src/pulse_model/calculations.py`, `src/pulse_model/quantum_source_response.py`, `src/pulse_model/causal_pulse_response.py`, `src/pulse_model/no_free_branch_variance.py`, `src/pulse_model/stochastic_pulse_geometry.py`, `src/pulse_model/quantum_pulse_geometry.py`, `src/pulse_model/pulse_closure_response.py`, `tests/test_h6_branch_decoherence.py`, `tests/test_quantum_source_response.py`, `tests/test_causal_pulse_response.py`, `tests/test_no_free_branch_variance.py`, `tests/test_stochastic_pulse_geometry.py`, `tests/test_quantum_pulse_geometry.py`, `tests/test_pulse_closure_response.py` | Use H6 only as a bounded branch-distinguishability, source-to-metric comparison, and source-response diagnostic framework; H6S3 means branch pulse-count variance is not physical merely from ensemble notation. H6S4-C may be used as a finite stochastic variance comparator, not as an accepted stochastic classical law. H6S4-Q may be used as a finite quantum mediator comparator and conditional derivation, not as an accepted Pulse Model quantum geometry law. H6P1 may be used as a finite closure-response diagnostic, not as a conserved source-response law. Stronger emergence is not on an internal live path unless a new external principle supplies conservation, coefficient, and baseline-distinct observable structure. |
...
The H6S2 appendix strengthens H6S1 by adding ensemble-decomposition invariance. It proves that branch-mixture response is not an admissible spacelike law merely because a branch decomposition can be written down. Branch-conditioned variance, correlation, and visibility become valid targets only when the branch basis is supported by local pointer records, a causal selection rule, or comparison in a shared causal future. H6S2's final verdict is **diagnostic tool** with a separate clean no-go subtheorem; at that stage, the pulse-native law was still unsupplied.

The H6S3 appendix adds the no-free-branch-variance theorem and route ledger. It proves that physical branch pulse-count variance cannot be obtained merely by choosing one ensemble decomposition of a fixed local density operator. Its final verdict is **diagnostic tool** with a separate clean no-go subtheorem. H6S3 does not choose H6S4 and does not supply the pulse-native law.

The H6S4-C appendix tests the stochastic classical pulse-geometry route. It defines a finite spectral-potential stochastic response, implements deterministic response and baseline-comparison helpers, proves by focused tests that equivalent decompositions of the same density operator give the same stochastic response, and separates density-only, invalid ensemble, pointer-record, ordinary-noise, and H6S3 baselines. Its final verdict is **diagnostic tool**: the finite excess-variance comparator is useful, but H6S4-C does not supply a general conserved stochastic classical source-response law distinct from known stochastic semiclassical gravity.

The H6S4-Q appendix tests the non-classical mediator route. It defines a finite source-mediator-probe setup, implements deterministic joint-record and marginal helpers, proves by focused tests that equivalent decompositions of the same density operator give the same local probe marginal, and compares density-only, H6S4-C, pointer/decoherence, fixed-background, linearized quantum-gravity, and quantum-mediator baselines. Its final verdict is **conditional derivation**: the finite record contract recovers known quantum mediator structure in Pulse Model language, but H6S4-Q does not supply a pulse-native quantum geometry law or fixed observable distinct from standard quantum mediator baselines.

The H6P1 appendix tests the pulse-closure response route. It defines gauge-invariant closed pulse or phase defects, an intrinsic covariance-weighted closure functional, and a finite response-gradient analogue, then checks orientation reversal, orientation-even functional behavior, independent-loop additivity, pure gauge relabeling, ensemble-decomposition invariance, missing covariance, missing conservation, fitted-coefficient rejection, ordinary-noise separation, and H6S4-C/Q baselines. Its final verdict is **diagnostic tool**: the finite closure machinery is useful for testing future covariance or mediator proposals, but H6P1 does not supply a conserved tensor response, a continuum intrinsic closure covariance law, a new pulse-native coefficient, or a fixed observable distinct from the H6S4-C stochastic and H6S4-Q quantum mediator baselines.

The stronger emergence claim remains blocked. H6 does not yet define a physical gauge-invariant quantum Hilbert space of metrics, a physical rule for coherent metric superposition beyond known mediator models, a collapse or branch-selection law, a no-signaling state-update rule, general energy-momentum accounting through stochastic or selected response, a conserved closure-response source, or a generic macroscopic derivation of one classical GR spacetime. H6 may use H5 as a bounded matter-side branch-distinguishability input and 05/05S as conditional low-energy source-to-metric support only when their assumptions are carried explicitly.
...
3. Use H6, H6S1, H6S2, H6S3, and H6P1 downstream only as bounded branch-distinguishability, source-to-metric comparison, no-free-branch-variance, closure-response, and source-response diagnostic frameworks; do not treat the reduced toy models, ensemble-invariance guardrail, route ledger, or closure functional as a full emergence proof or a real source-response law.
